// lib/screens/canteen_transactions_screen.dart
...
class CanteenTransactionsScreen extends StatefulWidget {
  ...
  @override
  State<CanteenTransactionsScreen> createState() =>
      _CanteenTransactionsScreenState();
}

class _CanteenTransactionsScreenState
    extends State<CanteenTransactionsScreen> {
  ...
  Future<void> _loadTransactions() async {
    ...
    if (_filterType == 'all') {
      ...
      result = await _apiService.getCanteenTransactionsByMonth(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildFilterOption(
                        'All Transactions', 'all', Icons.list_alt),
                    const SizedBox(height: 8),
                    _buildFilterOption('Today', 'today', Icons.today),
                    const SizedBox(height: 8),
                    _buildCustomFilterOption(
                        'Specific Date', Icons.calendar_today, () {
  ...
  Widget _buildCustomFilterOption(
  ...
  Widget _buildSummaryCard() {
    // FIX: Use correct keys matching backend canteen transaction response.
    // Backend returns: { totalDebit: "string", count: number }
    // We derive totalRevenue by parsing totalDebit, count from transactions length.
    final totalRevenue = double.tryParse(
            _summary?['totalDebit']?.toString() ?? '0') ??
        0.0;
  ...
  Widget _buildTransactionCard(Transaction transaction) {
    final displayName =
        transaction.name ?? transaction.username ?? 'Unknown User';
